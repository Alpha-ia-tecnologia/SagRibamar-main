...
export const SelectGrupo = ({ regiaoId, value, onChange }: Props) => {
  ...
  const handleCheckboxChange = (grupoId: string) => {
    if (value.includes(grupoId)) {
        ...
        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
          {grupos.map((grupo) => (
            ...
            >
              <input
                type="checkbox"
                checked={value.includes(grupo.id.toString())}
                onChange={() => handleCheckboxChange(grupo.id.toString())}
