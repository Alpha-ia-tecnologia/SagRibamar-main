...
export const SelectRegiao = ({ value, onChange }: Props) => {
  ...
  const handleCheckboxChange = (regiaoId: string) => {
    if (value.includes(regiaoId)) {
      onChange(value.filter(id => id !== regiaoId));
    } else {
      onChange([...value, regiaoId]);
    }
  };
        ...
        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
          {regioes.map((regiao) => (
            ...
            >
              <input
                type="checkbox"
                checked={value.includes(regiao.id.toString())}
                onChange={() => handleCheckboxChange(regiao.id.toString())}
                className="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
              <span className="text-sm text-gray-700">{regiao.nome}</span>
            </label>
          ))}
        </div>
      )}
