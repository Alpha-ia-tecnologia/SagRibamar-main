...
export const CreateQuestoesModal = ({ provaId, tituloProva, onClose, onSuccess }: CreateQuestoesModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!provaIdAtual && tituloProva) {
      try {
        const provaRes = await fetch(`${window.__ENV__?.API_URL}/api/provas`, {
          method: "POST",
    ...
    try {
      const res = await fetch(`${window.__ENV__?.API_URL}/api/questoes`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      {showModalBNCC && (
        <ModalBNCC
          ...
          onSelect={async (habilidades, profId) => {
            ...
            if (profId) {
              try {
                const res = await fetch(`${window.__ENV__?.API_URL}/api/bncc/${habilidades[0]?.id}/proficiencias`);
                if (res.ok) {
                  const data = await res.json();
